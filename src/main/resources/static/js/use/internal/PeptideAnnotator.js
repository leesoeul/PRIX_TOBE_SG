...
myApp.controller(
  ...
  function (
    ...
    $scope.peptide = {
      ...
      prositModel: parseInt($scope.getUrlVars().ce)
        ? $scope.getUrlVars().prositModel
          ? $scope.getUrlVars().prositModel
          : "intensity_prosit_publication"
        : "",
    };
    ...
    $scope.peptideBottom = {
      ...
      prositModel: parseInt($scope.getUrlVars().ce_bottom)
        ? $scope.getUrlVars().prositModel_bottom
          ? $scope.getUrlVars().prositModel_bottom
          : "intensity_prosit_publication"
        : "",
    };
    ...
    $scope.tableColumns = [{ id: 1, name: "M/Z, Intensity" }];
    ...
    $scope.loadMods = function (topSpectrum = true) {
      ...
      if ($scope.predeterminedMods.length == 0) {
        ...
        $http.get("/mods/Modifications.txt").then(
          function (data) {
            ...
            if (topSpectrum) {
              $scope.potentialMods = $scope.userMods.concat(
                $scope.predeterminedMods
              );
              $scope.mods = [];
            } else {
              $scope.potentialModsBottom = $scope.userMods.concat(
                $scope.predeterminedMods
              );
              $scope.modsBottom = [];
            }

            var sSeq = topSpectrum
              ? $scope.peptide.sequence
              : $scope.peptideBottom.sequence;

            var aTempPotMods = topSpectrum
              ? $scope.potentialMods
              : $scope.potentialModsBottom;
    ...
    $scope.openModalLoading = function () {
      $uibModal
        .open({
          templateUrl: "support/html/ModalTemplateLoading",
          scope: $scope,
    ...
    $scope.openModalConfirmation = function (message, topSpectrum = true) {
      $uibModal
        .open({
          templateUrl: "support/html/ModalTemplateConfirmation",
          scope: $scope,
          controller: function ($scope, $uibModalInstance, $localStorage) {
            $scope.confMessage = message;
            $scope.closePopUp = function () {
              let acrd = topSpectrum
                ? $scope.accordionTop
                : $scope.accordionBottom;
              acrd.manual = true;
              acrd.usi = false;
    ...
    $scope.openHelp = function () {
      $uibModal
        .open({
          templateUrl: "support/html/helpOverlay",
          scope: $scope,
          controller: function ($scope, $uibModalInstance, $localStorage) {
    ...
    $scope.openModal = function () {
      $uibModal
        .open({
          templateUrl: "support/html/ModalTemplateNew",
          scope: $scope,
          controller: function ($scope, $uibModalInstance, $localStorage) {
              ...
              if ($scope.potentialUserMod.name) {
                if ($scope.userModSites.selected.length > 0) {
                  if ($scope.potentialUserMod.deltaMass != 0) {
                    ...
                    alert(
                      "Please enter a modification mass shift. This can be positive or negative"
                    );
                  }
                } else {
            ...
            $scope.clearMods = function () {
              if (
                confirm(
                  "Are you sure you want to clear all user mods from your history? This cannot be undone."
                )
              ) {
    ...
    $scope.cutoffs = {
      ...
      toleranceType: ["ppm", "Da"].includes(
        $scope.getUrlVars().fragment_tol_unit
      )
        ? $scope.getUrlVars().fragment_tol_unit
        : "ppm",
      tolerance: parseInt($scope.getUrlVars().fragment_tol) || 10,
      compToleranceType: ["ppm", "Da"].includes(
    ...
    $scope.validateSequence = function (topSpectrum = true) {
      ...
      for (var i = 0; i < sSequence.length; i++) {
        ...
        if (!regex.exec(character)) {
          ...
          alert(
            character +
              " aaa is not a valid amino acid. Only the 20 canonical amino acids are supported."
          );
        }
      }
    };
    ...
    $scope.validateCharge = function (topSpectrum = true) {
      if (topSpectrum) {
        ...
        $scope.peptide.precursorCharge = Math.round(
        ...
        $scope.peptideBottom.precursorCharge = Math.round(
        ...
        // set fragment charge min and max from the precursor charge
        if ($scope.peptideBottom.precursorCharge > 0) {
          $scope.peptideBottom.fragmentMax =
            $scope.peptideBottom.precursorCharge; // TODO: check
          $scope.peptideBottom.fragmentMin = 1;
          $scope.checkModel.a.label = "a";
        } else {
          $scope.peptideBottom.fragmentMax = -1;
          $scope.peptideBottom.fragmentMin =
            $scope.peptideBottom.precursorCharge + 1;
          $scope.checkModel.a.label = "a\u2022";
        }
...
myApp.controller("HotCtrlBottom", function ($scope) {
  $scope.afterInitBottom = function () {
    setTimeout(function (x) {
      $scope.handsonTableInstanceBottom = this;
    }, 1000);
  };
...
myApp.directive("handsontabletest", function () {
  return {
    templateUrl: "support/html/HotTableTemplate" /*,
  ...
  function ($scope, $log) {
    $scope.modSelectOption = function (mod, topSpectrum = true) {
      ...
      if (
        (mod.index != -1 &&
          topSpectrum &&
          mod.index != $scope.peptide.sequence.length) ||
        (!topSpectrum && mod.index != $scope.peptideBottom.sequence.length)
      ) {
