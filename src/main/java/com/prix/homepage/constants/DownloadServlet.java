...
public class DownloadServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			Integer pathInt = Integer.parseInt(request.getParameter("path"));

			Data dataResult = dataMapper.getNameContentById(pathInt);
			InputStream is = new ByteArrayInputStream(dataResult.getContent());
			...
			if (summary.getFileName().lastIndexOf('\\') != -1) {
				prefix = summary.getFileName().substring(summary.getFileName().lastIndexOf('\\') + 1,
						summary.getFileName().lastIndexOf('.'));
			} else if (summary.getFileName().lastIndexOf('/') != -1) {
				prefix = summary.getFileName().substring(summary.getFileName().lastIndexOf('/') + 1,
						summary.getFileName().lastIndexOf('.'));
			} else
				prefix = summary.getFileName().substring(0, summary.getFileName().lastIndexOf('.'));
	...
	public static Element get_search_summary(ProteinSummary proSummary, boolean isDBbond) {
		...
		// enzyme.setAttribute(new Attribute( "cleave",
		// Constants.userDBEnzyme.getCleave() ));
		// enzyme.setAttribute(new Attribute( "term",
		// Constants.userDBEnzyme.getCleavageSide()+"term" ));
		summary.addContent(enzyme);

		Element parameters = new Element("parameters");
		Element enzyme_constraint = new Element("enzyme_constraint");
		enzyme_constraint
				.setAttribute(new Attribute("max_miss_cleavages", String.valueOf(proSummary.getMaxMissedCleavages())));
		// enzyme_constraint.setAttribute( new Attribute( "min_number_termini",
		// String.valueOf(proSummary.getMinTerminiNumber()) ) );
		...
		peptide_mass_tol
				.setAttribute(new Attribute("unit", (proSummary.getPTUnit().compareToIgnoreCase("ppm") == 0) ? "ppm" : "Da"));
		...
		if (!isDBbond) {
			...
			modified_mass_range
					.setAttribute(new Attribute("min_value", String.format("%.1f", proSummary.getProteinMassMin())));
			modified_mass_range
					.setAttribute(new Attribute("max_value", String.format("%.1f", proSummary.getProteinMassMax())));
	...
	public static Element get_peptide_hit_element(ProteinSummary summary, int rank, PeptideInfo peptide) {
		...
		peptide_hit.setAttribute(
				new Attribute("prev_next_res", String.valueOf(peptide.getNTerm(0)) + String.valueOf(peptide.getCTerm(0))));
		// peptide_hit.setAttribute(new Attribute( "next_aa",
		// String.valueOf(peptide.getCTerm(0)) ));
	...
	public static Element get_comonent_hit_element(ProteinSummary summary, String rank, PeptideInfo alpha) {
		...
		peptide_hit.setAttribute(
				new Attribute("prev_next_res", String.valueOf(alpha.getNTerm(0)) + String.valueOf(alpha.getCTerm(0))));
	...
	public static void write_pept_spectrum_match(PrintWriter out, ProteinSummary summary, SpectrumInfo spectrum) {
		...
		if (peptide.getModIndices().length > 0) {
			...
			int[] mod = peptide.getModOffsets();
			for (int k = 0; k < site.length; k++) {
				modified.append(String.format("%s(%c%d) ", summary.getModifications()[mod[k]].getType(),
						peptide.getName().charAt(site[k] - 1), site[k]));
			}
		}
		...
		for (int j = 0; j < peptide.getProteinCount(); j++) {
			...
			out.printf("%.4f,%.4f,%.4f,%s,%d,%d,%c.%s.%c,%s\r\n",
					peptide.getMassCalc(), peptide.getMassDelta(), peptide.getScore(),
					summary.getProtein(peptide.getProteinIndex(j)).getName(), peptide.getStartPosition(j),
					peptide.getEndPosition(j),
	...
	public static void write_bond_spectrum_match(PrintWriter out, ProteinSummary summary, SpectrumInfo spectrum) {
		...
		StringBuffer curatedPept = new StringBuffer(
				peptide.getNTerm(0) + "." + peptide.getName() + "." + peptide.getCTerm(0));
		int bondSite = 0;
		int[] site = peptide.getModIndices();
		int[] mod = peptide.getModOffsets();
		String modified = "";
		for (int k = 0; k < site.length; k++) {
			if (mod[k] == 0) {
				...
				modified = String.format("%s(%c%d) ", summary.getModifications()[mod[k]].getType(),
						peptide.getName().charAt(site[k] - 1), site[k]);
			}
		}
		/*
		...
		out.printf("%s,%d,%s,%s,", summary.getProtein(peptide.getProteinIndex(0)).getName(),
				peptide.getStartPosition(0) + bondSite,
		...
		out.printf("%s,%d,%s,%s\r\n", summary.getProtein(peptide.getProteinIndex(0)).getName(),
				peptide.getStartPosition(0) + bondSite,
				curatedPept.toString(), modified);
