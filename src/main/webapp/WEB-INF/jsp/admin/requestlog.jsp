...
<%
	...
	String swroot = "/usr/local/server/apache-tomcat-8.0.14/webapps/ROOT/download/software_archive/release";
	String download_root = "https://prix.hanyang.ac.kr/download/software_archive/release";
	int success = 0;
	int update = 0, delete = 0;
	String userid = "";

	if (level == null || id == null) level = -1;

	if (request.getParameter("accept") != null && level > 1) {
		update = 1;
		userid = request.getParameter("accept");
	}
	else if (request.getParameter("reject") != null && level > 1) {
		update = 2;
		userid = request.getParameter("reject");
	}
	else if (request.getParameter("delete") != null && level > 1) {
		delete = 1;
		userid = request.getParameter("delete");
	}

	if ( update > 0 && conn != null && level > 1) {
				...
				if ( rs.next() ) {
					...
					for( File file : protDir.listFiles() ) {
						...
						if( fileLowerCase.startsWith( software.toLowerCase() ) ){
							// 
							// isFile = true: send email with a file.
							// isFile = false: send email without a file, but it includes a link can connect to file.
							...
							if(isFile){
								...
								message = (rs.next())? rs.getString(1) : "";
								rs = state.executeQuery("select message from px_software_msg where id='signature';");
								signature= (rs.next())? rs.getString(1) : "";
							}else{
								rs = state.executeQuery("select message from px_software_msg where id='" + software.toLowerCase() + "_link';");
								message = (rs.next())? rs.getString(1) : "";
								rs = state.executeQuery("select message from px_software_msg where id='signature';");
								signature= (rs.next())? rs.getString(1) : "";
							}
							...
							if(isFile){
								...
								String link_path = download_root+"/"+file.getName();
								message = message.replace("<link>", link_path);
